...
function ChoiceGlyph({ choice }: { choice: string }) {
  if (choice === "red" || choice === "black") {
    return (
      <span
        className={`flex h-16 w-12 items-center justify-center rounded-md border-2 text-2xl ${
          choice === "red"
  ...
  return <>{SYMBOL_GLYPHS[choice] ?? <span className="text-2xl">?</span>}</>;
}
...
const DRILL_TITLES: Record<string, string> = {
  zener4: "Which symbol is it?",
  redblack: "Red or black?",
};

export default function DrillClient({ drillType }: { drillType: string }) {
  const [trial, setTrial] = useState<Trial | null>(null);
  const [result, setResult] = useState<GuessResult | null>(null);
  const [confidence, setConfidence] = useState(drillType === "redblack" ? 50 : 25);
  ...
  const chancePct = trial ? Math.round(trial.chance * 100) : drillType === "redblack" ? 50 : 25;
  const gridCols = drillType === "redblack" ? "grid-cols-2" : "grid-cols-2 sm:grid-cols-4";
      ...
      <header className="flex items-center justify-between">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-muted">
            Quick-fire · chance {chancePct}%
          </p>
          <h1 className="mt-1 text-xl font-semibold">
            {DRILL_TITLES[drillType] ?? "Take a guess"}
          </h1>
        </div>
        ...
        <section className="rounded-2xl border border-card-border bg-card/80 p-6">
          ...
          <div className={`mt-5 grid gap-3 ${gridCols}`}>
            {(trial?.choices ?? []).map((choice) => (
              <button
                ...
                onClick={() => void submitGuess(choice)}
            ...
            {!trial &&
              !error &&
              Array.from({ length: drillType === "redblack" ? 2 : 4 }).map((_, i) => (
                <div
                  key={i}
                  className="h-28 animate-pulse rounded-xl border border-card-border bg-black/20"
                />
              ))}
          </div>
          ...
          <div className="mt-6">
            ...
            <p className="mt-1 text-xs text-muted">
              Honest confidence earns calibration XP. Chance here is {chancePct}%.
            </p>
          </div>
        </section>
      )}
      ...
      {result && (
        <section className="rounded-2xl border border-card-border bg-card/80 p-6">
          <div className="flex items-center gap-4">
            <div
              className={`reveal-flip flex h-20 w-16 items-center justify-center rounded-xl border ${
                result.correct
                  ? "border-[var(--accent-phosphor)] text-[var(--accent-phosphor)]"
                  : "border-card-border text-muted"
              }`}
            >
              <ChoiceGlyph choice={result.answer} />
            </div>
            <div>
              ...
              <p className="text-sm text-muted">
                +{result.xpAwarded} XP
                {result.calibrationXp > 0 ? ` (incl. +${result.calibrationXp} calibration)` : ""} ·{" "}
                {result.level}
              </p>
            </div>
          </div>
